...
export default function Signup() {
  ...
  const [step, setStep] = useState(1); // 1=account type, 2=details, 3=stylist subscription
  ...
  async function handleSubmit() {
    ...
    if (acct === "stylist" && !specialty) { setError("Specialty is required for stylist accounts."); return; }
    ...
    <div className="screen" style={{ paddingBottom: 0 }}>
      ...
      <div className="body">
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 16 }}>
          <div onClick={() => fileRef.current.click()} style={{ width: 80, height: 80, borderRadius: "50%", background: photoPreview ? "transparent" : "var(--pink-light)", border: "2px dashed var(--pink)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", overflow: "hidden", marginBottom: 6 }}>
            {photoPreview
              ? <img src={photoPreview} alt="profile" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
              : <i className="ti ti-camera" style={{ fontSize: 24, color: "var(--pink)" }} aria-hidden="true"></i>
            }
          </div>
          <span style={{ fontSize: 12, color: "var(--text-secondary)" }}>Add profile photo (optional)</span>
        ...
        <textarea
          ...
          placeholder={`About me ${acct === "stylist" ? "* (required for stylists)" : "(optional)"}`}
          value={about}
        ...
        {acct === "stylist" && (
          <>
            <input className="input-field" placeholder="Your styling specialty * (e.g. Business casual, Streetwear)" value={specialty} onChange={e => setSpecialty(e.target.value)} />
