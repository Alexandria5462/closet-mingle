...
export default function Following() {
  ...
  // Client: load stylists they follow
      ...
      const withProfiles = await Promise.all(
        follows.map(async f => {
          try {
            ...
            return { ...f, stylist: stylistSnap.exists() ? stylistSnap.data() : null };
  ...
  // Stylist: load clients who follow them
  async function loadFollowers() {
    ...
    try {
      ...
      const withProfiles = await Promise.all(
        follows.map(async f => {
          try {
            const clientSnap = await getDoc(doc(db, "users", f.followerId));
            return { ...f, client: clientSnap.exists() ? clientSnap.data() : null };
    ...
    <>
      <div className="header">
        ...
        <span className="badge" style={{ background: "var(--avatar-bg)", color: "var(--pink-dark)" }}>
          {isStylist ? followers.length : following.length} {isStylist ? "follower" : "following"}{(isStylist ? followers.length : following.length) !== 1 ? "s" : ""}
        </span>
      </div>
        ...
        <div className="body">
          ...
          {loading ? (
            ...
            followers.length === 0 ? (
              ...
              <div key={f.followId} style={{ background: "var(--bg-card)", border: "0.5px solid var(--border)", borderRadius: "var(--radius)", padding: 14, marginBottom: 10 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div className="avatar" onClick={() => nav(`/stylist/client/${f.followerId}`)} style={{ width: 48, height: 48, fontSize: 15, overflow: "hidden", flexShrink: 0, cursor: "pointer" }}>
                    {f.client?.photoUrl
            ...
            following.length === 0 ? (
              ...
              <div key={f.followId} style={{ background: "var(--bg-card)", border: "0.5px solid var(--border)", borderRadius: "var(--radius)", padding: 14, marginBottom: 10 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div className="avatar" onClick={() => nav(`/stylist/${f.stylistId}`)} style={{ width: 52, height: 52, fontSize: 16, overflow: "hidden", flexShrink: 0, cursor: "pointer" }}>
                    {f.stylist?.photoUrl
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, flexShrink: 0 }}>
                    ...
                    <button onClick={() => unfollow(f.followId, f.stylist?.name)} style={{ background: "none", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "5px 10px", cursor: "pointer", fontSize: 11, color: "var(--text-secondary)", fontFamily: "inherit" }}>
                      Unfollow
                    </button>
                  </div>
                </div>
