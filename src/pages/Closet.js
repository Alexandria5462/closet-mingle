...
export default function Closet() {
  ...
  async function handlePhoto(file) {
    ...
    setUploading(true);
    try {
      const storageRef = ref(storage, `closets/${userProfile.uid}/${Date.now()}_${file.name}`);
          ...
          {loading ? (
            ...
            <div className="closet-grid">
              ...
              <div className="closet-item" style={{cursor:"pointer",border:"2px dashed var(--border)"}} onClick={()=>setShowCamera(true)}>
                <div className="closet-item-placeholder"><i className="ti ti-plus" style={{fontSize:28,color:"var(--pink)"}} aria-hidden="true"></i><span style={{color:"var(--pink)"}}>Add</span></div>
              </div>
            </div>
          )}
        </div>
