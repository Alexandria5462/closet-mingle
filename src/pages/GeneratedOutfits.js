...
async function generateOutfitsFromLiked(likedItems, occasion) {
 try {
   const descriptions = likedItems.map(item => ({
     ...
     fit: item.attributes?.fit || "standard",
   }));
   const response = await fetch("https://api.anthropic.com/v1/messages", {
     ...
     body: JSON.stringify({
       ...
       messages: [{
         role: "user",
         content: `You are an expert fashion stylist AI. The user has liked these clothing items:
${JSON.stringify(descriptions, null, 2)}
Occasion: "${occasion}"
Create 3 DIFFERENT complete outfit combinations using ONLY these items.
For each outfit you MUST:
1. Include a top/outerwear + bottom OR a dress (required)
2. Add shoes if available
3. Add accessories only if they complement the outfit
4. Match colors using fashion rules:
  - Complementary colors (opposite on color wheel)
  - Analogous colors (adjacent on color wheel)
  - Neutral + pop of color
  - Monochromatic (same color family)
5. Never pair two busy patterns together
6. Consider the occasion and style attributes
Return ONLY this JSON array:
[
 {
   "outfitName": "Creative stylish name",
   "selectedIds": ["id1", "id2", "id3"],
   "colorStory": "Specific explanation of WHY these colors work together e.g. navy and cream create a timeless contrast",
   "patternNote": "How the patterns complement each other",
   "styleNote": "Why this works for the occasion",
   "colorHarmony": "complementary/analogous/neutral/monochromatic"
 }
]
Make all 3 outfits different combinations. Only return the JSON array.`
       }]
     })
   });
   ...
   return outfits.map(outfit => ({
     ...outfit,
     items: outfit.selectedIds.map(id => likedItems.find(i => i.id === id)).filter(Boolean)
